...
func (m SignedURLAuthenticator) signatureIsValid(req *http.Request) (err error) {
	...
	signingKey := m.deriveSigningKey(userFromCtx)
...
// This function must match waht's defined in internal/http/services/owncloud/ocs/handlers/cloud/user/user.go
func (m SignedURLAuthenticator) deriveSigningKey(u *userv1beta1.User) []byte {
	bytesKey := argon2.Key([]byte(m.config.SigningKeySecret), []byte(u.Username), 3, 32*1024, 4, 32)
...
func (m SignedURLAuthenticator) buildUrlToSign(req *http.Request) string {
	...
	urlToSign := *req.URL
	//encodedPath := url.PathEscape(urlToSign.Path)
	//urlToSign.Path = encodedPath
	if len(q) == 0 {
...
func (m SignedURLAuthenticator) Authenticate(r *http.Request) (*http.Request, bool) {
	...
	userResp, err := client.GetUserByClaim(ctx, &userv1beta1.GetUserByClaimRequest{
		Claim: "username",
		Value: r.URL.Query().Get(_paramOCCredential),
	})
	// TODO(jgeens): check userResp
	if err != nil {
		log.Error().
			Err(err).
	...
	// TODO(jgeens): do we need?

	// user, err := m.UserRoleAssigner.ApplyUserRole(r.Context(), userResp.User)
	// if err != nil {
	// 	log.Error().
	// 		Err(err).
	// 		Str("authenticator", "signed_url").

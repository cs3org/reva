...
func (s *svc) handleSpacesProppatch(w http.ResponseWriter, r *http.Request, spaceID string) {
	ctx := r.Context()
	ctx, span := trace.StartSpan(ctx, "spaces_proppatch")
	defer span.End()
...
func (s *svc) handleProppatch(ctx context.Context, w http.ResponseWriter, r *http.Request, ref *provider.Reference, patches []Proppatch, log zerolog.Logger) (accepted []xml.Name, removed []xml.Name, ok bool) {
	c, err := s.getClient()
	if err != nil {

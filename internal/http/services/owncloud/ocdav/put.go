...
func (s *svc) handlePathPut(w http.ResponseWriter, r *http.Request, ns string) {
	...
	s.handlePut(ctx, w, r, ref, fn, sublog)
}

func (s *svc) handlePut(ctx context.Context, w http.ResponseWriter, r *http.Request, ref *provider.Reference, path string, log zerolog.Logger) {
	...
	ok, err := chunking.IsChunked(path)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
	if ok {
		chunk, err := chunking.GetChunkBLOBInfo(path)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
...
func (s *svc) handleSpacesPut(w http.ResponseWriter, r *http.Request, spaceID string) {
	...
	s.handlePut(ctx, w, r, spaceRef, spaceRef.GetId().GetOpaqueId(), sublog)
}

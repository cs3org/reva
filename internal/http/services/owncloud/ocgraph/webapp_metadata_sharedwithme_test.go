...
func TestReceivedShareErrorLogOmitsSharedSecret(t *testing.T) {
	const (
		shareID         = "ocm-share-log"
		syntheticSecret = "synthetic-shared-secret-marker"
	)
	share := receivedOCMShareWithWebapp(shareID, "remote.txt", "CodiMD")
	share.Protocols[0].GetWebdavOptions().SharedSecret = syntheticSecret
	share.Protocols[1].GetWebappOptions().SharedSecret = syntheticSecret

	buf, logger := captureLogger()
	ctx := appctx.ContextSetUser(appctx.WithLogger(context.Background(), &logger), &userpb.User{
		Id: &userpb.UserId{OpaqueId: "me"},
	})
	gw := &receivedSharesGateway{
		ocmShares: []*ocm.ReceivedShare{share},
		user: &userpb.User{
			Id:          &userpb.UserId{OpaqueId: "creator"},
			DisplayName: "Creator",
		},
	...
	stampGateway(gw)
	stampGatewayErrorAfter(2, errors.New("forced gateway failure for ocm conversion"))
	s := sharedWithMeService(t)
	req := httptest.NewRequest(http.MethodGet, "/me/drive/sharedWithMe", nil).WithContext(ctx)
	rec := httptest.NewRecorder()
	s.getSharedWithMe(rec, req)
	...
	if !bytes.Contains(buf.Bytes(), []byte(shareID)) {
		t.Fatalf("log missing shareId %q: %s", shareID, buf.Bytes())
	}
	if bytes.Contains(buf.Bytes(), []byte(syntheticSecret)) {
		t.Fatalf("log contains shared secret: %s", buf.Bytes())
	}
}

func TestGetSharedWithMeDuplicateWebappProtocolsOmitExtension(t *testing.T) {
	share := receivedOCMShareWithWebapp("ocm-share-dup", "remote.txt", "CodiMD")
	share.Protocols = append(share.Protocols, &ocm.Protocol{
		Term: &ocm.Protocol_WebappOptions{
			WebappOptions: &ocm.WebappProtocol{
				Uri:          "https://remote.example/open/second",
				SharedSecret: "secret-second-must-stay-in-storage",
				AppName:      "Etherpad",
			},
		},
	})
	if receivedShareWebappMetadata(share) != nil {
		t.Fatal("duplicate webapp protocols produced metadata")
	}

	body := sharedWithMeOCM(t, share)
	if bytes.Contains(body, []byte(receivedWebappJSONKey)) {
		t.Fatalf("duplicate webapp body contains %s: %s", receivedWebappJSONKey, body)
	}
	values := decodeSharedWithMeValues(t, body)
	if len(values) != 1 {
		t.Fatalf("value length = %d, want 1", len(values))
...
func TestGetSharedWithMeMalformedWebappProtocolOmitsExtension(t *testing.T) {
	share := receivedOCMShareInfo()
	share.Id = &ocm.ShareId{OpaqueId: "ocm-share-malformed"}
	share.Name = "remote.txt"
	share.Protocols = append(share.Protocols, &ocm.Protocol{
		Term: &ocm.Protocol_WebappOptions{WebappOptions: nil},
	})
	if receivedShareWebappMetadata(share) != nil {
		t.Fatal("malformed webapp protocol produced metadata")
	}

	body := sharedWithMeOCM(t, share)
	if bytes.Contains(body, []byte(receivedWebappJSONKey)) {
		t.Fatalf("malformed webapp body contains %s: %s", receivedWebappJSONKey, body)
	}
	values := decodeSharedWithMeValues(t, body)
	if len(values) != 1 {
		t.Fatalf("value length = %d, want 1", len(values))
...
func TestGetSharedWithMeEmptyAppNameExposesBlankSibling(t *testing.T) {
	...
	base := mustJSON(t, drive)
	assertWebappSibling(t, base, values[0], "")
	if !bytes.Contains(remoteItemOf(t, values[0]), []byte(`"appName":""`)) {
		t.Fatalf("remoteItem missing blank appName: %s", remoteItemOf(t, values[0]))
	}
	if bytes.Contains(body, []byte(`"appName":null`)) {
		t.Fatalf("blank appName encoded as null: %s", body)
	}
}

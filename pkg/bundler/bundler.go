...
// ErrorMode selects how Create reacts to failures
type ErrorMode int

const (
	// ErrorFailFast aborts on the first file error
	ErrorFailFast ErrorMode = iota

	// TODO: implement, uncertain how
	ErrorCollect
)
...
type Options struct {
	...
	ZipMethod uint16
	// Errors selects the failure behavior
	Errors ErrorMode
}
...
// FileError records one skipped file in ErrorCollect mode
type FileError struct {
	Path string
	Err  error
}

// CollectedErrors is the error returned in ErrorCollect mode when at least one file was skipped
...
// Error returns the string error msg for CollectedErrors
func (e *CollectedErrors) Error() string {
	return fmt.Sprintf("bundler: %d files could not be bundled", len(e.Errors))
}

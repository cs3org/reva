...
func (fs *cephfs) Upload(ctx context.Context, ref *provider.Reference, r io.ReadCloser) error {
	...
	if ok {
		...
		user.op(func(cv cacheVal) {
			r, err = cv.mount.Open(assembledFile, os.O_RDONLY, filePermDefault)
		})
		if err != nil {
...
func (fs *cephfs) NewUpload(ctx context.Context, info tusd.FileInfo) (upload tusd.Upload, err error) {
	...
	log.Debug().Interface("info", info).Msg("cephfs: resolved filename")
...
func (fs *cephfs) getUploadPath(ctx context.Context, uploadID string) (string, error) {
	return filepath.Join(fs.conf.Uploads, uploadID), nil
}

// GetUpload returns the Upload for the given upload id
func (fs *cephfs) GetUpload(ctx context.Context, id string) (tusd.Upload, error) {
	usr := fs.makeUser(ctx)
...
func (upload *fileUpload) WriteChunk(ctx context.Context, offset int64, src io.Reader) (n int64, err error) {
	...
	user.op(func(cv cacheVal) {
		file, err = cv.mount.Open(upload.binPath, os.O_WRONLY|os.O_APPEND, filePermDefault)
	})
	if err != nil {
		return 0, err
	}
...
func (upload *fileUpload) FinishUpload(ctx context.Context) (err error) {
	...
	if err != nil {
		if !os.IsNotExist(err) {
			log := appctx.GetLogger(ctx)
			log.Err(err).Interface("info", upload.info).Msg("cephfs: could not delete upload info")
		}
	}

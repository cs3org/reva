...
func (fs *s3ngfs) RemoveGrant(ctx context.Context, ref *provider.Reference, g *provider.Grant) (err error) {
	...
	if g.Grantee.Type == provider.GranteeType_GRANTEE_TYPE_GROUP {
		attr = xattrs.GrantPrefix + xattrs.GroupAcePrefix + g.Grantee.GranteeId.GetUserId().OpaqueId
	} else {
		attr = xattrs.GrantPrefix + xattrs.UserAcePrefix + g.Grantee.GranteeId.GetUserId().OpaqueId
	}

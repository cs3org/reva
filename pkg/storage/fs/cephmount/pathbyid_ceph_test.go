...
func TestCephRootConfiguration(t *testing.T) {
	...
	t.Run("fstab_configuration", func(t *testing.T) {
		...
		if !exists {
			t.Fatal("No fstab entry available - set CEPHMOUNT_FSTAB_ENTRY environment variable")
		}
...
func TestGetPathByIDWithCreatedFiles(t *testing.T) {
	// This test creates actual files and directories in the Ceph filesystem using cephmount,
	// extracts their inodes, and then queries them via the admin connection using GetPathByID
	RequireCephIntegration(t)

	// Use the integration helper to get cephmount FS with real fstab config
	ctx := ContextWithTestLogger(t)
	fs := CreateCephMountFSForIntegration(t, ctx, nil)
	...
	t.Log("  1. Files and directories can be created in the Ceph filesystem via cephmount")

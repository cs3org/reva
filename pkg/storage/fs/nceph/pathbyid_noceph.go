...
func newCephAdminConn(ctx context.Context, conf *Options) (*CephAdminConn, error) {
	return nil, errtypes.NotSupported("nceph: ceph support not enabled (build with -tags ceph)")
}
...
func (fs *ncephfs) GetPathByID(ctx context.Context, id *provider.ResourceId) (string, error) {
	return "", errtypes.NotSupported("nceph: GetPathByID requires ceph support (build with -tags ceph)")
}

...
func (fs *Eosfs) AddGrant(ctx context.Context, ref *provider.Reference, g *provider.Grant) error {
	...
	if err != nil {
		log.Error().Err(err).Msgf("FindMe failed to get eos acl")
	...
	log.Info().Any("ref", ref).Str("path", fn).Msgf("FindMe calling AddACL")

	err = fs.c.AddACL(ctx, sysAuth, fn, eosclient.StartPosition, eosACL)
	if err != nil {
		log.Error().Err(err).Any("ref", ref).Str("path", fn).Msgf("FindMe Failed to call AddACL")

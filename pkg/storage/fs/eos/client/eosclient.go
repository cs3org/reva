...
type EOSClient interface {
	AddACL(ctx context.Context, auth Authorization, path string, position uint, a *acl.Entry, recursive bool) error
	RemoveACL(ctx context.Context, auth Authorization, path string, a *acl.Entry, recursive bool) error
	UpdateACL(ctx context.Context, auth Authorization, path string, position uint, a *acl.Entry, recursive bool) error
	...
	ListAllQuota(ctx context.Context, rootAuth Authorization) (map[string]*QuotaInfo, error)
...
func NobodyRole() Role {
	return Role{
		UID: strconv.Itoa(int(nobody)),
		GID: strconv.Itoa(int(nobody)),
	}
}

...
func (d *driver) Upload(ctx context.Context, ref *provider.Reference, content io.ReadCloser, _ storage.UploadFinishedFunc) (provider.ResourceInfo, error) {
	...
	err = d.unwrappedOpFromShareCreator(ctx, share, rel, func(ctx context.Context, newRef *provider.Reference) error {
...
func (d *driver) GetPathByID(ctx context.Context, id *provider.ResourceId) (string, error) {
	info, err := d.GetMD(ctx, &provider.Reference{ResourceId: id}, nil, nil)
	if err != nil {
		return "", err
	}

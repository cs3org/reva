...
func createReceivedShareTables(ctx *sql.Context, initData []*ocm.ReceivedShare) map[string]*memory.Table {
	...
	for _, share := range initData {
		...
		must(tableShares.Insert(ctx, sql.NewRow(mustInt(share.Id.OpaqueId), share.Name, share.ResourceId.StorageId, share.ResourceId.OpaqueId, share.Grantee.GetUserId().OpaqueId, fmt.Sprintf("%s@%s", share.Owner.OpaqueId, share.Owner.Idp), fmt.Sprintf("%s@%s", share.Creator.OpaqueId, share.Creator.Idp), share.Ctime.Seconds, share.Mtime.Seconds, expiration, int8(convertFromCS3OCMShareType(share.ShareType)), int8(convertFromCS3OCMShareState(share.State)))))
...
func TestStoreReceivedShare(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: storeReceivedShareExpected{
				shares:    []sql.Row{{int64(1), "file-name", "storage", "opaque-id", "marie", "einstein@cernbox", "einstein@cernbox", uint64(1670859468), uint64(1670859468), nil, int8(ShareTypeUser), int8(ShareStateAccepted)}},
		...
		},
		{
			...
			expected: storeReceivedShareExpected{
				shares: []sql.Row{
					{int64(1), "file-name", "storage", "opaque-id", "marie", "einstein@cernbox", "einstein@cernbox", uint64(1670859468), uint64(1670859468), uint64(0), int8(ShareTypeUser), int8(ShareStateAccepted)},
					{int64(2), "file-name", "storage", "new-resource", "richard", "marie@cernbox", "marie@cernbox", uint64(1670859468), uint64(1670859468), nil, int8(ShareTypeUser), int8(ShareStatePending)},
				},
				protocols: []sql.Row{

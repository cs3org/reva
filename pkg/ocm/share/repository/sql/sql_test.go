	...
	for _, share := range initData {
		...
		for _, m := range share.AccessMethods {
			...
			case *ocm.AccessMethod_WebdavOptions:
				...
				must(webdav.Insert(ctx, sql.NewRow(i, int64(conversions.RoleFromResourcePermissions(am.WebdavOptions.GetPermissions(), false).OCSPermissions()))))
	...
	for _, share := range initData {
		...
		for _, p := range share.Protocols {
			...
			case *ocm.Protocol_WebdavOptions:
				...
				must(webdav.Insert(ctx, sql.NewRow(i, prot.WebdavOptions.Uri, prot.WebdavOptions.SharedSecret, int64(conversions.RoleFromResourcePermissions(prot.WebdavOptions.GetPermissions().GetPermissions(), false).OCSPermissions()))))
			...
			shares: []*ocm.Share{
				{
					...
					AccessMethods: []*ocm.AccessMethod{share.NewWebDavAccessMethod(conversions.NewEditorRole(false).CS3ResourcePermissions())},
				},
			},
			...
			fields: []*ocm.UpdateOCMShareRequest_UpdateField{
				{
					Field: &ocm.UpdateOCMShareRequest_UpdateField_AccessMethods{
						AccessMethods: share.NewWebDavAccessMethod(conversions.NewEditorRole(false).CS3ResourcePermissions()),
					},
				},
				{
					Field: &ocm.UpdateOCMShareRequest_UpdateField_AccessMethods{

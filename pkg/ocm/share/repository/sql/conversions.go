...
func convertToCS3OCMShare(s *dbShare, am []*ocm.AccessMethod) *ocm.Share {
	granteeUserId, _ := ocmd.GetUserIDFromOCMUser(s.ShareWith)
	share := &ocm.Share{
		Id: &ocm.ShareId{
...
func convertToCS3OCMReceivedShare(s *dbReceivedShare, p []*ocm.Protocol) *ocm.ReceivedShare {
	ownerUserId, _ := ocmd.GetUserIDFromOCMUser(s.ShareWith)
	creatorUserId, _ := ocmd.GetUserIDFromOCMUser(s.Initiator)

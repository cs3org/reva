...
func webappViewModeToResourcePermissions(viewMode string) *provider.ResourcePermissions {
	role := permissions.NewViewerRole()
	if viewMode == "write" {
		role = permissions.NewEditorRole()
	}
	return role.CS3ResourcePermissions()
}
...
func (sm *Manager) efssShareToOcm(resp *EfssShare) *ocm.Share {
	...
	if resp.Protocols.WebApp.ViewMode != "" {
		am = append(am, share.NewWebappAccessMethod(
			webappViewModeToResourcePermissions(resp.Protocols.WebApp.ViewMode),
...
func efssReceivedShareToOcm(resp *ReceivedEfssShare) *ocm.ReceivedShare {
	...
	if resp.Share.Protocols.WebApp.ViewMode != "" {
		proto = append(proto, share.NewWebappProtocol(resp.Share.Protocols.WebApp.URI, resp.Share.Token,
			webappViewModeToResourcePermissions(resp.Share.Protocols.WebApp.ViewMode),

...
func getWebAppProtocol(appURL string, role string) *ocm.Protocol {
	var perms *ocm.SharePermissions
	switch role {
	case "viewer":
		perms = &ocm.SharePermissions{Permissions: permissions.NewViewerRole().CS3ResourcePermissions()}
	case "editor":
		perms = &ocm.SharePermissions{Permissions: permissions.NewEditorRole().CS3ResourcePermissions()}
	default:
		return nil
	}
...
func getOcmAccessMethods(role string) []*ocm.AccessMethod {
	...
	case "editor":
		return []*ocm.AccessMethod{
			...
			share.NewWebappAccessMethod(
				&ocm.SharePermissions{Permissions: permissions.NewEditorRole().CS3ResourcePermissions()},
				share.DefaultWebappRequirements, share.DefaultWebappTargets, ""),
		}
	}
	return []*ocm.AccessMethod{}

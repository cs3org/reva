...
type dbReceivedShare struct {
	...
	Mtime         int
	Expiration    sql.NullInt64
	Type          model.ShareType
	State         model.OcmShareState
}
...
func convertToCS3Protocol(p *model.OcmReceivedShareProtocol) *ocm.Protocol {
	...
	case model.WebappProtocol:
		return share.NewWebappProtocol(p.UriTemplate, appprovider.ViewMode(p.ViewMode))
	case model.TransferProtocol:
		return share.NewTransferProtocol(p.SourceUri, p.SharedSecret, uint64(p.Size))
	}
	return nil
}

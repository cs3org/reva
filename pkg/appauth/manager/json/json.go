...
func (mgr *jsonManager) GenerateAppPassword(ctx context.Context, scope map[string]*authpb.Scope, label string, expiration *typespb.Timestamp) (*apppb.AppPassword, error) {
	...
	if _, ok := mgr.passwords[userID.OpaqueId]; !ok {
		mgr.passwords[userID.OpaqueId] = make(map[string]*apppb.AppPassword)
	}

	mgr.passwords[userID.OpaqueId][password] = appPass
...
func (mgr *jsonManager) ListAppPasswords(ctx context.Context) ([]*apppb.AppPassword, error) {
	...
	for _, pw := range mgr.passwords[userID.OpaqueId] {
...
func (mgr *jsonManager) InvalidateAppPassword(ctx context.Context, password string) error {
	...
	appPasswords, ok := mgr.passwords[userID.OpaqueId]
	...
	delete(mgr.passwords[userID.OpaqueId], password)

	// if user has 0 passwords, delete user key from state map
	if len(mgr.passwords[userID.OpaqueId]) == 0 {
		delete(mgr.passwords, userID.OpaqueId)
	}

	return mgr.save()
}

// reload re-reads the JSON file into memory. Must be called with mgr.Lock held.
func (mgr *jsonManager) reload() {
	data, err := os.ReadFile(mgr.config.File)
	if err != nil {
		return
	}
	passwords := make(map[string]map[string]*apppb.AppPassword)
	if json.Unmarshal(data, &passwords) == nil {
		mgr.passwords = passwords
	}
}
...
func (mgr *jsonManager) GetAppPassword(ctx context.Context, userID *userpb.UserId, password string) (*apppb.AppPassword, error) {
	...
	// Reload from disk so passwords generated by other service instances are visible.
	mgr.reload()

	appPassword, ok := mgr.passwords[userID.OpaqueId]
	if !ok {
